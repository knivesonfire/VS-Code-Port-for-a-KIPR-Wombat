...
typedef struct Controller {
    ...
    * @param velocity_cm_per_sec linear wheel velocity in centimeters per second
     */
    void (*forward)(float dist, float velocity_cm_per_sec);
    ...
    * @param velocity_cm_per_sec linear wheel velocity in centimeters per second
     */
    void (*backward)(float dist, float velocity_cm_per_sec);
    ...
    * @param velocity_cm_per_sec  linear outer-wheel velocity in centimeters per second
     */
    void (*left)(int angle, float radius, float velocity_cm_per_sec);
    ...
    * @param velocity_cm_per_sec  linear outer-wheel velocity in centimeters per second
     */
    void (*right)(int angle, float radius, float velocity_cm_per_sec);

    /**
    * Set a motor target velocity using mav units (ticks per second)
    * @param motor                 the motor port to command
    * @param velocity_ticks_per_sec velocity in ticks per second
     */
    void (*motor)(int motor, int velocity_ticks_per_sec);
 ...
 * @param  motor_left_inverted      non-zero if left motor direction should be inverted
 * @param  motor_right_inverted     non-zero if right motor direction should be inverted
